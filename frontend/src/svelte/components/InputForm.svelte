			...
			>
				...
				{#if $isMobile}
					<span class="helper-text"
						>Hãy thử các ví dụ. Dùng máy tính để nhập trực tiếp.</span
					>
				{:else if $isLoaded && $isFetchingModel}
					<span class="helper-text"
						>Hãy thử các ví dụ trong khi mô hình GPT-2 đang được tải (525MB)</span
					>
				{:else if exceedLimit}
					<span class="helper-text">Bạn có thể nhập tối đa {wordLimit} từ.</span>
				{/if}
			</div>
		</ButtonGroup>
		<button
