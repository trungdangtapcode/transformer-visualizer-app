<script lang="ts">
	...
	// color scale — spread to unwrap Svelte 5 proxy from d3.extent result
	$: qkColorScaleDomain = [...d3.extent(queryKey.flat())].map(v => Number.isFinite(v) ? +v : 0);
	$: qkColorScale = (d, i) => {
		return d3
			.scaleLinear()
	...
	>
		...
		>
			...
			<div class="color-scale">
				<span class="val">{Number(qkColorScaleDomain[0]).toFixed(1)}</span>
				<div class="bar"></div>
				<span class="val">{Number(qkColorScaleDomain[1]).toFixed(1)}</span>
			</div>
		</div>
		<!-- Scaling · Mask -->
		<div
